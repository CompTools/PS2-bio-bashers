...
question3_function () {
  ...
  cat unique_airport_codes.txt
  # The list of airport codes in alphabetical order is outputted in the file "unique_airport_codes.txt"
}
question3_function 
# The above line executes the function within the bash script and thus produces the list of airport codes in the file "unique_airport_codes.txt"

# Problem 4 solution
question4_function () {
  cat HW2_dataset.csv | grep -oE '[[:alpha:]]{1,}, FL' | sort -u > Unique_FL_airports.txt 
  cat Unique_FL_airports.txt
